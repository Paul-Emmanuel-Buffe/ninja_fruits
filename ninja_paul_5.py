...
def New_Game(screen,image, start_time, game_duration, score, game_over, missed_fruits, speed, player_name, language):
    ...
    while running:
            ...
            if score > 0 and remaining_time == 0:
                running = False
                win_text = LARGE_FONT.render(texts[language]["you_win"], True, (255, 255, 255))
...
def main():
    ...
    game_over = False
    running = True
    language = "en"  # Défini ici au lieu d'utiliser une variable globale
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if state_screen == Main_Menu:
                    if rect1.collidepoint(event.pos):
                        ...
                        state_screen = Exit
                    elif language_rect.collidepoint(event.pos):  # Vérifie si on clique sur le bouton "language"
                        language = "fr" if language == "en" else "en"  # Bascule entre anglais et français
